...
- 固定 Release 中 Android APK 的文件名为 `tlm-app.apk`。Flet 0.28 默认输出 `app-release.apk`，容易导致下载时看不到原来的 APK 名称。
...
- 针对手机端灰屏加固：Android 打包从 Flet 0.85.3 回退并锁定为更成熟的 `flet==0.28.3` / `flet-cli==0.28.3`，避开 0.85 Android 模板连接 Python 页面时卡灰屏的问题。
- 保留 v2.1.3 的兼容改动，主界面拟合图继续使用纯 Python PNG + Flet `Image`，不依赖 `LineChart`。
- 新增 `src/assets/icon.png`，兼容 Flet 0.28 的图标查找方式。
